...
class YOLOPerception(PerceptionModule):
    ...
    def get_semantic_vis(self, semantic_map, palette):
    ...
    def overlay_semantics(self, rgb_image, object_semantic_vis, receptacle_semantic_vis):
        ...
        receptacle_mask = (np.array(receptacle_semantic_vis_pil) != 0).astype(np.uint8) * 255
    ...
    def get_combined_semantic_vis(self, rgb_image, object_mask, receptacle_mask, palette):
        rgb_image_pil = Image.fromarray(rgb_image)

        object_semantic_vis = self.get_semantic_vis(object_mask, palette)
        receptacle_semantic_vis = self.get_semantic_vis(receptacle_mask, palette)

        combined_vis = self.overlay_semantics(rgb_image_pil, object_semantic_vis, receptacle_semantic_vis)
    ...
    def save_image_sequence(self, image, base_path="image_dump/15"):
    ...
    def predict(
        ...
        nms_threshold=0.8
        seg_tensor = obs["ovmm_nav_goal_segmentation"].cpu().squeeze().numpy()
        og_object_mask =seg_tensor[..., 0:1]
        og_recep_mask = seg_tensor[..., 1:2]
        all_recep = obs["receptacle_segmentation"].cpu().squeeze().numpy()
        ...
        for idx, result in enumerate(results):
            ...
            # self.save_image_sequence(svimage , "og_image_dump/8")
            ...
            if class_ids.size !=0:
                ...
                all_recep_image = cv2.addWeighted(image, 1 - alpha, color_map, alpha, 0)
                # cv2.imwrite('new_cmap_all_recep_mask{}.png'.format(idx), color_map)
                # cv2.imwrite('new_all_recep_mask{}.png'.format(idx), all_rec_mask)
                palette = [174, 199, 232, 255, 127, 14, 255, 187, 120, 44, 160, 44, 
                152, 223, 138, 214, 39, 40, 255, 152, 150, 148, 103, 189, 197, 176, 213, 
                140, 86, 75, 196, 156, 148, 227, 119, 194, 247, 182, 210, 127, 127, 127, 
                199, 199, 199, 188, 189, 34, 219, 219, 141, 23, 190, 207, 158, 218]
                combined_vis = self.get_combined_semantic_vis(image, obj_semantic_mask, rec_semantic_mask, palette)
                combined_image = self.concatenate_images(combined_vis, svimage)

                
                # scaled_og_object_mask = cv2.resize(og_object_mask[0], (480, 640), interpolation=cv2.INTER_LINEAR)[ ..., np.newaxis]
                # scaled_og_recep_mask = cv2.resize(og_recep_mask[0], (480, 640), interpolation=cv2.INTER_LINEAR)[ ..., np.newaxis]
                # all_recep = all_recep.astype(np.float32)
                # scaled_all_recep_mask = cv2.resize(all_recep, (480, 640), interpolation=cv2.INTER_LINEAR)

                all_recep_img = (all_recep * 255 / all_recep.max()).astype(np.uint8)
                all_recep_img = cv2.applyColorMap(all_recep_img, cv2.COLORMAP_JET)
                image = cv2.resize(image,(120, 160), interpolation=cv2.INTER_AREA)
                og_combined_vis = self.get_combined_semantic_vis(image, og_object_mask, og_recep_mask, palette)


                og_combined_image = self.concatenate_images(og_combined_vis,all_recep_img)
                og_combined_image = np.array(Image.fromarray(og_combined_image).resize((960, 640), Image.ANTIALIAS))

                concatenated_image = np.concatenate(( og_combined_image, combined_image), axis=0)
                # self.save_image_sequence(combined_image)
                # # Add the text for the target object
                cv2.putText(concatenated_image, f"Target object: {CLASSES[obj_class_ids[idx]]}", (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.85, (0,0,0), 2)

                # # Add the text for the target receptacle
                cv2.putText(concatenated_image, f"Target Receptacle: {CLASSES[rec_class_ids[idx]]}", (20, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.85, (0,0,0), 2)
                # self.save_image_sequence(concatenated_image )
                # self.save_image_sequence(resized_image, "og_image_dump/7")
                # cv2.imwrite('all_mask{}.png'.format(idx), combined_image)
                ...
                iou_object = self.calculate_iou(obj_semantic_mask, og_object_mask)
                iou_recep = self.calculate_iou(rec_semantic_mask, og_recep_mask)
                # logger.info(f"IOU Object: {iou_object}, IOU Receptacle: {iou_recep}")            
                ...
                all_recep = np.expand_dims(all_recep, axis=-1)

                iou_all_recep = self.calculate_iou(all_rec_mask, all_recep)
                logger.info(f"IOU Object: {iou_object}, IOU Receptacle: {iou_recep}, IOU All Receptacle: {iou_all_recep}")
                cv2.putText(concatenated_image, f"IOU Object: {iou_object}, Receptacle: {iou_recep}", (20, 1250), cv2.FONT_HERSHEY_SIMPLEX, 0.85, (0,0,0), 2)
                cv2.putText(concatenated_image, f"IOU All Receptacle: {iou_all_recep}", (600, 1250), cv2.FONT_HERSHEY_SIMPLEX, 0.85, (0,0,0), 2)
...
class GazeResNetEncoder(nn.Module):
    def __init__(
        ...
        self.segmentation = YOLOPerception(sem_gpu_id=0, verbose=False, confidence_threshold=0.35)
        self.masks = torch.zeros((1, 160, 120, 3))  # Adjust to the batch size
    ...
    def forward(self, observations: Dict[str, torch.Tensor]) -> torch.Tensor:  # type: ignore
        ...
        for k in self.visual_keys:
            ...
            if(k == "head_depth"):
                ...
                depth_tensor_resized = F.interpolate(obs_k, size=(160, 120), mode='nearest')
                # depth_tensor_resized_np = depth_tensor_resized.cpu().numpy()
                obs_k = depth_tensor_resized.permute(0, 2, 3, 1)
                obs_k = torch.tensor(obs_k, device=torch.device('cuda:{}'.format(torch.cuda.current_device()))).detach()
