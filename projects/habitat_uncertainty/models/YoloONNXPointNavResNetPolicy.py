...
class YoloONNXPointNavResNetPolicy(NetPolicy):
    def __init__(
        self,
...
class YOLOPerception(PerceptionModule):
    def __init__(
        ...
        """
        yolo_model_id="yolov8x-worldv2.onnx",
        ...
        with torch.no_grad():
            self.model = YOLO("yolov8x-worldv2.onnx")
        # vocab = CLASSES
    ...
    def save_image_sequence(self, image, base_path="image_dump/19"):
    ...
    def predict(
        ...
        for idx, image in enumerate(images):
            results = list(self.model(image, classes=search_list, conf=self.confidence_threshold,  iou=nms_threshold, stream=True, verbose=False))
            result= results[0]
...
class ResNetEncoder(nn.Module):
    def __init__(
        ...
        if not self.is_blind:
            ...
            self.masks = torch.zeros((32, 160, 120, 3))  # Adjust to the batch size
