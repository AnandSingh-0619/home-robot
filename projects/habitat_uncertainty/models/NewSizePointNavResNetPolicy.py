...
class NewSizePointNavResNetPolicy(NetPolicy):
    def __init__(
        self,
    ...
    def __init__(
        ...
        if not self.visual_encoder.is_blind:
            self.depth_fc = nn.Sequential(
                nn.Flatten(),
                nn.Linear(
                    np.prod(self.depth_encoder.output_shape), hidden_size
                ),
                nn.ReLU(True),
            )
        ...
        self.state_encoder = build_rnn_state_encoder(
            (0 if self.is_blind else 2*self._hidden_size) + rnn_input_size,
            self._hidden_size,
            rnn_type=rnn_type,
