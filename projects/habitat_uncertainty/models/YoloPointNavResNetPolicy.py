...
class YOLOPerception(PerceptionModule):
    def __init__(
        ...
        if verbose:
            ...
            logger.info(f"Total number of parameters in YOLO model: {total_yolo_params}")
        ...
        for idx, result in enumerate(results):
            ...
            if class_ids.size !=0:
                ...
                if self.save_images:
                    ...
                    combined_vis = self.get_combined_semantic_vis(image, obj_semantic_mask, rec_semantic_mask)
                    concatenated_image = self.concatenate_images(combined_vis, color_map)
...
class ResNetEncoder(nn.Module):
    def __init__(
        ...
        if not self.is_blind:
            ...
            self.segmentation = YOLOPerception(yolo_model_id=self.config.yolo_model_id, sem_gpu_id=0, verbose=self.config.verbose, confidence_threshold=0.35, save_images = self.config.save_images, save_path=self.config.save_path)
            self.masks = torch.zeros((self.config.num_env, 160, 120, 3))  # Adjust to the batch size
    ...
    def forward(self, observations: Dict[str, torch.Tensor]) -> torch.Tensor:  # type: ignore
        ...
        for k in self.visual_keys:
            ...
            obs_k = F.interpolate(obs_k, size=(120, 120), mode='bilinear', align_corners=False)
...
class PointNavResNetNet(Net):
    ...
    def __init__(
        ...
        if self.policy_config.combine_encoders:
            final_channels=( (self.visual_encoder.output_shape[0] if self.policy_config.use_visual_encoder else 0) +
                (self.depth_encoder.output_shape[0] if self.policy_config.use_depth_encoder else 0)
            )
        ...
        spatial_dims = (
            self.visual_encoder.output_shape[1] if self.policy_config.use_visual_encoder
        ...
        self.final_visual_fc = nn.Sequential(
                    nn.Flatten(),
                    nn.Linear(
                        compression_channels * spatial_dims * spatial_dims, hidden_size
                    ),
                    nn.ReLU(True),
                )
