...
JOB_ID=${SLURM_JOB_ID}
CHECKPOINT_DIR="data/new_checkpoints/navObj_nPolicy_yolo_${JOB_ID}"
TENSORBOARD_DIR="Logs/tensorLogs/navObj_nPolicy_yolo_${JOB_ID}"
LOG_DIR="Logs/logs/navObj_nPolicy_yolo_${JOB_ID}.log"

source ~/.bashrc
source /srv/kira-lab/share4/asingh3064/miniforge3/etc/profile.d/conda.sh
...
srun python -um habitat_uncertainty.run \
    --exp-config=projects/habitat_uncertainty/config/nav_to_obj_yolo.yaml \
    --run-type=train \
    ...
    habitat_baselines.rl.ddppo.use_detector=True \
    habitat_baselines.rl.policy.use_yolo=True
